...
- we propose a novel approach to generate multiple feasible hypotheses of the 3D pose from 2D joints. In contrast to existing deep learning approaches which minimize a mean square error based on an unimodal Gaussian distribution, our method is able to generate multiple feasible hypotheses of 3D pose based on a multimodal mixture density networks. 
- we follow [12] to compute the MPJPE between the ground truth and the best 3D hypothesis generated by our network
...
- soft-argmax
- $$J = \sum_x \sum_y \sum_z p\cdot H_k(p)$$, $$p$$ all locations in 3D cube, $$H_k(p)$$ heatmap for the k-th joint.
